<script lang="ts">
  ...
  function speak(text: string = '你') {
    ...
    const length = cnVoice.length;
    if (length === 0) {
    }
    const randomIndex = Math.floor(Math.random() * length);
    const randomVoice = cnVoice[randomIndex];

    const u = new SpeechSynthesisUtterance(text);

    if (randomVoice) u.voice = randomVoice;

    u.lang = randomVoice.lang;
    console.log('🚀 ~ speak ~ u:', u);
    ...
    try {
      if (sheetMode === 'sample') {
        ...
        if (!spreadsheetId || !apiKey)
          throw new Error('Vui lòng nhập Spreadsheet ID và API Key');
  ...
  function saveSettings() {
    localStorage.setItem(
      'fc_settings',
      JSON.stringify({ sheetMode, spreadsheetId, apiKey, sheetName, csvUrl })
    );
    loadCards();
    showSettings = false;
  }
...
<main class="app">
  ...
  <header>
    ...
    <nav>
      <button
        class="nav-btn"
        class:active={studyMode === 'all'}
        on:click={() => {
          store.studyMode.set('all');
          store.reset();
        }}
      >
      ...
      <button
        class="nav-btn"
        class:active={studyMode === 'starred'}
        on:click={() => {
          store.studyMode.set('starred');
          store.reset();
        }}
      >
      ...
      <button
        class="nav-btn"
        class:active={studyMode === 'unlearned'}
        on:click={() => {
          store.studyMode.set('unlearned');
          store.reset();
        }}
      >
        Chưa học <span class="badge"
          >{allCards.filter((c) => !learnedIds.has(c.id)).length}</span
        >
      </button>
    </nav>
    <div class="header-actions">
      ...
      <button
        class="icon-btn"
        title="Cài đặt"
        on:click={() => (showSettings = true)}>⚙</button
      >
    </div>
  </header>
      ...
      <div class="progress-wrap">
        <div class="progress-info">
          <span class="progress-count"
            >{currentIndex + 1} / {activeCards.length}</span
          >
      ...
      <div class="card-area">
        ...
        <div class="card-actions">
          ...
          <div class="nav-btns">
            <button
              class="nav-arrow"
              on:click={() => store.prev(activeCards)}
              title="Trước (←)">←</button
            >
            <button
              class="nav-arrow"
              on:click={() => store.next(activeCards)}
              title="Tiếp (→)">→</button
            >
          </div>
          <button
    ...
    >
      <div class="modal">
        ...
        <div class="mode-tabs">
          <button
            class="mode-tab"
            class:active={sheetMode === 'sample'}
            on:click={() => (sheetMode = 'sample')}
          >
            Demo
          </button>
          <button
            class="mode-tab"
            class:active={sheetMode === 'api'}
            on:click={() => (sheetMode = 'api')}
          >
            Google Sheets API
          </button>
          <button
            class="mode-tab"
            class:active={sheetMode === 'csv'}
            on:click={() => (sheetMode = 'csv')}
          >
            CSV Public
          </button>
        </div>
        ...
        {:else if sheetMode === 'api'}
          <div class="info-box">
            <p>
              📋 Kết nối trực tiếp với Google Sheets qua API v4. Sheet phải có
              cột: <code>Front</code>, <code>Back</code> (và tuỳ chọn:
              <code>Category</code>, <code>Hint</code>)
            </p>
          </div>
          <label class="field">
            ...
            />
            <small
              >Lấy từ URL: docs.google.com/spreadsheets/d/<b>[ID]</b
              >/edit</small
            >
          </label>
          <label class="field">
        ...
        {:else}
          <div class="info-box">
            <p>
              🔗 Dùng Google Sheet đã publish dạng CSV (không cần API key). Vào <b
                >File → Share → Publish to web → CSV</b
              > để lấy link.
            </p>
          </div>
          <label class="field">
